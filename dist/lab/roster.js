// Fictionalized prototype assignments, not historical claims or active NPC simulation.
...
export function rosterRows({faction='all',sort=-1,descending=true}={}){
 const rows=officers.filter(o=>faction==='all'||o.side===faction);
 if(sort>=0)rows.sort((a,b)=>(a.stats[sort]-b.stats[sort])*(descending?-1:1)||a.id-b.id);
 return rows;
}
export function rosterMarkup(selected=0,options={}){
 const rows=rosterRows(options),o=rows.find(x=>x.id===selected)||rows[0];
 const face=(o,large=false)=>o.name==='袁术'?`<span class="officer-face ${large?'large':''}"><img src="assets/yuan-shu.png" alt="袁术"></span>`:`<span class="officer-face monogram ${large?'large':''}" aria-hidden="true">${o.name[0]}</span>`;
 ...
 return `<div class="roster-toolbar"><div><strong>荆襄武将</strong><span>${rows.length} / ${officers.length} 人</span></div><label>势力 <select id="rosterFaction"><option value="all" ${options.faction==='all'?'selected':''}>全部势力</option>${['刘表军','袁术军'].map(s=>`<option ${options.faction===s?'selected':''}>${s}</option>`).join('')}</select></label></div><div class="officer-browser"><section class="officer-ledger" aria-label="武将能力列表"><div class="officer-table-scroll"><table class="officer-table"><thead><tr><th scope="col">武将</th><th scope="col">势力</th><th scope="col">所在</th>${th}<th scope="col">职责</th></tr></thead><tbody>${rows.map(x=>`<tr data-officer="${x.id}" class="${x.id===o.id?'is-selected':''}"><th scope="row"><button data-officer="${x.id}" aria-pressed="${x.id===o.id}" aria-label="预览${x.name}">${face(x)}<span>${x.name}</span></button></th><td><span class="faction-dot ${x.side==='袁术军'?'enemy':''}"></span>${x.side.replace('军','')}</td><td>${x.place}</td>${x.stats.map(v=>`<td class="ability-value ${v>=85?'excellent':''}">${v}</td>`).join('')}<td class="duty-cell">${x.role.split(' · ')[0]}</td></tr>`).join('')}</tbody></table></div><p class="ledger-hint">点击一行预览武将 · 点击能力列排序</p><p class="ledger-note">能力为本原型暂定值（满值 100），尚未参与战斗结算。角色编组为架空演练。</p></section><section class="officer-preview" aria-label="武将预览"><div class="preview-identity">${face(o,true)}<div><small>${o.side} · ${o.place}</small><h3>${o.name}</h3><p>${o.role}</p></div></div><div class="ability-bars">${o.stats.map((v,i)=>`<div><span>${abilityNames[i]}</span><b>${v}</b><i><em style="width:${v}%"></em></i></div>`).join('')}</div><dl class="preview-facts"><dt>职责</dt><dd>${o.work}</dd><dt>人物交互 · 规划</dt><dd>${o.talk}</dd></dl><details class="preview-more"><summary>自主行动与任务设计</summary><p>${o.ai}</p><p>${o.hook}</p></details><p class="preview-note">拜访、任务与 AI 出击尚未接入。</p></section></div>`;
}
