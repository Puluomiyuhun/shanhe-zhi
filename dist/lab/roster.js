...
export function rosterMarkup(selected=0,options={},simulation=null){
 ...
 const live=officer=>simulation?.actors.find(a=>a.name===officer.name);const place=officer=>{const a=live(officer);if(!a)return officer.place;const p=simulation.position(a);return cityAt(p.x,p.z)?.name||'赴'+a.destination;};
