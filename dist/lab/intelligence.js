...
export function createIntelligence({getWorld,getSimulation,inspectActor,cities,pause,focusCity,focusUnit,plan,standby}){
 ...
 function render(){
  ...
  let html='';
  if(view==='realm'){
   html='<div class="roster-toolbar"><div><strong>天下势力</strong><span>九方势力 · 演练配置</span></div><span>选择势力，查阅所属城市</span></div><div class="officer-table-scroll"><table class="officer-table army-ledger"><thead><tr><th>势力</th><th>区域</th><th>控制土地</th><th>城池</th><th>情报</th></tr></thead><tbody>';
   for(const f of factions){const count=w.cells.filter(c=>c.owner===f.id).length,controlled=cities.filter(c=>c.owner===f.id);html+=`<tr><th><i class="realm-dot" style="background:${f.color}"></i>${button('cities-'+f.id,f.name+(f.id===1?' · 所属':''))}</th><td>${f.region}</td><td>${count.toLocaleString()} 格</td><td>${controlled.length} 城</td><td>${button('cities-'+f.id,'所属城市 ›')}</td></tr>`;}
