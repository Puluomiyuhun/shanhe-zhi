...
export function createSidebar({getWorld,getSimulation,cities,onInspect,onPlan,onLocate,onUnit,onActor,onLocateActor,onOfficer}){
 ...
 function refresh(){
  ...
  const residentHTML='<small>城内人物 · '+resident.length+' 位</small>'+(hero?'<span class="resident-hero">主角 · 沈行舟</span>':'')+'<div class="resident-roster">'+resident.map(o=>'<button data-resident-officer="'+o.id+'" title="查看'+o.name+'">'+officerFace(o)+'<span>'+o.name+'<small>'+(liveOfficer(o,sim)?.phase||o.role.split(' · ')[0])+'</small></span></button>').join('')+'</div>';
