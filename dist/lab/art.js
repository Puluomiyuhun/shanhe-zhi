...
function texture(kind){
  ...
  const ctx=canvas.getContext('2d');ctx.fillStyle=kind==='roof'?'#8c918a':kind==='wall'?'#aeb0a2':'#b4b4a3';ctx.fillRect(0,0,256,256);
...
export function makeCity(city){
 const g=new THREE.Group();g.position.set(city.x,height(city.x,city.z),city.z);g.scale.setScalar(city.name==='新野'?.9:1.1);
...
export function addLandscape(scene){
 ...
 for(let i=0;i<26000;i++){const x=(random()-.5)*222,z=(random()-.5)*212,y=height(x,z),slope=Math.hypot(height(x+.3,z)-height(x-.3,z),height(x,z+.3)-height(x,z-.3));if(y>11.2||y<.3||slope>1.05||Math.abs(x-roadX(z))<2.4||cities.some(c=>Math.hypot(c.x-x,c.z-z)<4.4)||Math.abs(z-riverZ(x))<riverWidth(x)+1.08)continue;if(Math.sin(x*.27+z*.16)+Math.cos(z*.28-x*.13)<.5)continue;treePoints.push({x,y,z,s:.48+random()*.65});}
 ...
 const rockPoints=[];for(let i=0;i<3800;i++){const x=(random()-.5)*216,z=(random()-.5)*202,y=height(x,z);const bank=Math.abs(z-riverZ(x));if((y>3.8&&random()>.48)||(bank>riverWidth(x)+.53&&bank<riverWidth(x)+1.68&&Math.abs(x-roadX(z))>3))rockPoints.push({x,y,z,s:y>3.8?.3+random()*.75:.1+random()*.22});}
