...
export function makeCity(city){
 const g=new THREE.Group();g.position.set(city.x,height(city.x,city.z),city.z);g.scale.setScalar(city.name===format('{target}')?.9:1.1);
...
export function addLandscape(scene,onRoad=()=>false){
 ...
 for(let i=0;i<17000;i++){const x=(random()-.5)*(scenario.map.activeExtent||scenario.map.extent).x*2,z=(random()-.5)*(scenario.map.activeExtent||scenario.map.extent).z*2,y=height(x,z),slope=Math.hypot(height(x+.3,z)-height(x-.3,z),height(x,z+.3)-height(x,z-.3));if(y>11.2||y<.3||slope>1.05||onRoad(x,z)||cities.some(c=>Math.hypot(c.x-x,c.z-z)<4.4)||waterAt(x,z).shore<1.08)continue;if(Math.sin(x*.27+z*.16)+Math.cos(z*.28-x*.13)<.5)continue;treePoints.push({x,y,z,s:.48+random()*.65});}
 ...
 treePoints.forEach((t,i)=>{dummy.position.set(t.x,t.y,t.z);dummy.rotation.set(0,random()*6.28,0);dummy.scale.set(t.s,t.s*(.85+random()*.3),t.s);dummy.updateMatrix();leaves.setMatrixAt(i,dummy.matrix);leaves.setColorAt(i,new THREE.Color().setHSL(.205+random()*.08,.22+random()*.13,.22+random()*.12));dummy.position.y=t.y+.42*t.s;dummy.updateMatrix();trunks.setMatrixAt(i,dummy.matrix);});leaves.castShadow=leaves.receiveShadow=true;trunks.castShadow=true;trunks.userData.smallDetail=true;for(const mesh of [leaves,trunks]){mesh.userData.treePoints=treePoints;mesh.userData.originalMatrices=mesh.instanceMatrix.array.slice();}scene.add(leaves,trunks);
 const rockPoints=[];for(let i=0;i<2400;i++){const x=(random()-.5)*(scenario.map.activeExtent||scenario.map.extent).x*2,z=(random()-.5)*(scenario.map.activeExtent||scenario.map.extent).z*2,y=height(x,z);const bank=waterAt(x,z).shore;if((y>3.8&&random()>.48)||(bank>.53&&bank<1.68))rockPoints.push({x,y,z,s:y>3.8?.3+random()*.75:.1+random()*.22});}
