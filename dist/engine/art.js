...
export function addLandscape(scene,onRoad=()=>false){
 ...
 const rockPoints=[];for(let i=0;i<2400;i++){const x=(random()-.5)*scenario.map.extent.x*2,z=(random()-.5)*scenario.map.extent.z*2,y=height(x,z);const bank=waterAt(x,z).shore;if((y>3.8&&random()>.48)||(bank>.53&&bank<1.68))rockPoints.push({x,y,z,s:y>3.8?.3+random()*.75:.1+random()*.22});}
