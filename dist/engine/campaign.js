...
import {createInitialScenery} from './settlements.js';
...
export function createCampaign(world,onEvent=()=>{},options={}){
 ...
 function makeSlots(f,home){return [0,1,2].map(slot=>{
  ...
  const leader=officers[slot]||officers[0];
  return{id:'state-'+f.id+'-'+slot,name:slot===2?f.name+'工役队':leader?.name||f.ruler,owner:f.id,kind:'army',mission:'campaign',slot,command:leader?.stats[0]||60,active:false,troops:0,cell:home.cell,path:[],progress:0,phase:'整备',home:home.name,destination:home.name,reason:'等待国中军令',trips:0,steps:0,visits:{},morale:100,food:0,initialTroops:0,order:'attack',targetArmy:null,serial:0};
 });}
 const armies=factions.flatMap(f=>makeSlots(f,owned(f.id)[0]));
 ...
 function transfer(city,owner){
  ...
  for(const id of provinces.get(city.name)||[])if(world.cells[id].owner===old)world.cells[id].owner=owner;
  world.cells[city.cell].owner=owner;revision++;evolution.transferFacilities(city,owner);
  if(!owned(old).length&&!countries.get(old).wandering){countries.get(old).eliminated=true;for(const b of active().filter(b=>b.owner===old))dismiss(b);for(const r of relations.values())if(r.a===old||r.b===old){r.status='peace';r.until=0;}emit('elimination',faction(old).name+'失去全部城邑，退出本轮角逐。');}
 }
 function capture(a,city){
  const old=city.owner;transfer(city,a.owner);city.walls=25;city.order=45;city.governor=faction(a.owner).ruler;
  const garrison=Math.min(a.troops,Math.max(300,Math.floor(a.troops*.35)));a.troops-=garrison;city.garrison=garrison;
  stats.captures++;emit('capture',a.name+'攻克'+city.name+'，'+faction(old).name+' → '+faction(a.owner).name+'。');returnArmy(a,'战后回城休整');
 }
 function domestic(){for(const c of cities){const bonus=evolution.bonus(c);
 ...
 function battle(){const list=active(),engaged=new Set();
  ...
  for(const a of active())if(a.order==='attack'&&!engaged.has(a.id)){
   ...
   a.phase='攻城';const loss=Math.min(a.troops,Math.ceil(Math.sqrt(c.garrison)*(3+c.walls/40)*(1+evolution.bonus(c).defense))),defLoss=Math.min(c.garrison,Math.ceil(Math.sqrt(a.troops)*(4+a.command/40)/(1+c.walls/100)));
