...
export function cityEconomyMarkup(city){return `<div class="city-economy"><span>府库 <b>${num(city.gold)}</b></span><span>粮草 <b>${num(city.grain)}</b></span><span>守军 <b>${num(city.garrison)}</b></span><span>城防 <b>${num(city.walls)}</b></span><span>民心 <b>${num(city.order)}</b></span><span>内政 <b>${city.project}</b></span><span>首领 <b>${city.governor}</b></span><span>忠诚 <b>${Math.round(city.loyalty)}</b></span></div><div class="facility-ledger"><strong>城郊营建 <small>${city.facilities.filter(f=>f.status==='complete').length} / 3</small></strong>${city.facilities.map(f=>`<div><span>${FACILITY_TYPES[f.kind].name}</span><b>${f.status==='complete'?'已建成':f.progress?'施工 '+f.progress+'/'+f.required+' 旬':'工役队赴工'}</b><small>${FACILITY_TYPES[f.kind].effect}${f.status==='building'?' · 竣工后生效':''}</small></div>`).join('')||'<p>尚未营建 · 国家将按钱粮与军情派遣工役队</p>'}</div>`;}
