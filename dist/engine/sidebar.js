...
export function createSidebar({getWorld,getSimulation,cities,onInspect,onPlan,onLocate,onUnit,onActor,onLocateActor,onOfficer}){
 const $=id=>document.getElementById(id);let mode='unit',city=cities.find(c=>c.name===scenario.player.home),actorId=getSimulation().actors[0]?.id,cityCache=null;
 ...
 function refresh(){
  ...
  $('cityDescription').textContent=city.name===format('{home}')?'先遣旅从此出发。返回城址的行军仍需沿实际通路执行。':city.name===format('{target}')?format('{home}与{target}之间可沿平原通行；远行时注意维持补给。'):'可查看地形并预览抵近路线。城池攻防尚未接入。';
