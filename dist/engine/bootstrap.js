...
if(!id){
 ...
 try{
  ...
  const note=document.querySelector('#guide details p');if(note)note.textContent=scenario.description+' '+scenario.factions.length+'方势力、'+scenario.cities.length+'城、'+scenario.officers.length+'名人物、'+scenario.actors.length+'名活动角色。湖泊、海域与险山不可走，河流经津渡通行。沿海为现代轮廓适配的地理背景，并非古海岸精确复原；疆域以外的背景区域尚不可行军。国家内政、野战、攻城易主与外交已接入简化模拟；主角战斗、人物成长和存档尚未接入。';
