...
if(!id){
 ...
 try{
  ...
  const note=document.querySelector('#guide details p');if(note)note.textContent=scenario.description+' '+scenario.factions.length+'方势力、'+scenario.cities.length+'城、'+scenario.officers.length+'名人物。湖泊、海域与险山不可走，河流经津渡通行。沿海为现代轮廓适配的地理背景，并非古海岸精确复原；疆域以外的背景区域尚不可行军。国家会派工役队建设屯田、市集、兵站。兵站从所属城邑调入有限粮储，军队可集结、补粮休整，敌军会袭击邻近威胁兵站；首领偶尔叛变或自立，流浪军可择地筑城。城市情报显示忠诚与营建收益，军情记录事件。新国、新城为架空演进，不代表史实。主角战斗、人物成长和存档尚未接入。';
