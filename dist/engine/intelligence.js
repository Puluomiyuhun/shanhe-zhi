import {realmMarkup,diplomacyMarkup,cityEconomyMarkup} from './campaign-ui.js';
...
export function createIntelligence({getWorld,getSimulation,inspectActor,cities:initialCities,pause,focusCity,focusUnit,plan,standby}){
 const dialog=document.getElementById('intelligence'),content=document.getElementById('intelContent');
 const tabs=[['realm','势力'],['diplomacy','外交'],['cities','城市'],['officer','人物'],['roster',format('{officer}')],['unit','部队'],['journal','军情']];
 ...
 let cityFaction='all';
 let view='realm',cityName=scenario.player.home,entries=[],selectedOfficer=0;const rosterOptions={city:"all",faction:"all",sort:-1,descending:true};
 ...
 const escape=text=>String(text).replace(/[&<>\x22\x27]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
 ...
 function render(){
  ...
  let html='';
  if(view==='realm'){
   html=realmMarkup(getSimulation(),button);
   if(scenario.map.culturalRegions?.length)html+='<details class="cultural-ledger"><summary>外围族群 · '+scenario.map.culturalRegions.length+' 片分布区</summary><p class="intel-note">跨时期分布示意；轮廓经过概化。本次未配置城邑、人物和军队，暂不可行军或占领。</p>'+scenario.map.culturalRegions.map(r=>`<div><strong><i class="realm-dot" style="background:${r.color}"></i>${escape(r.name)}</strong><span>${escape(r.region)}</span><p>${escape(r.description)}</p></div>`).join('')+'</details>';
  }else if(view==='diplomacy'){html=diplomacyMarkup(getSimulation());
  }else if(view==='cities'){
   const city=cities.find(x=>x.name===cityName&&(cityFaction==='all'||String(x.owner)===cityFaction))||cities.find(x=>cityFaction==='all'||String(x.owner)===cityFaction);if(!city){content.innerHTML='<label>势力 <select id="cityFactionFilter"><option value="all">全部势力</option>'+factions.map(f=>`<option value="${f.id}" ${String(f.id)===cityFaction?'selected':''}>${f.name}</option>`).join('')+'</select></label><p class="empty-roster">该国已无城邑，请选择其他国家。</p>';return;}cityName=city.name;const target=w.nearest(city.x,city.z),route=routeTo(w,city),steps=route?route.path.length-1:null;
   ...
   html+='</tbody></table>';
   html+=`</div><section class="intel-card city-dossier"><div class="dossier-top"><span class="city-emblem" aria-hidden="true">城</span><div><small>${city.owner===scenario.player.owner?'己方据点':'对方据点'}</small><h3>${city.name}</h3><p>${owner(city.owner)}军</p></div></div><dl class="intel-stats">${datum('行军距离',steps===null?'无可达路线':steps===0?'已在城址':steps+' 格')}${datum('预计成本',route?route.cost.toFixed(1):'—')}${datum('城址地形',target.terrain)}${datum('出发部队',format('{hero} · {troops}'))}</dl>${getSimulation().campaign?cityEconomyMarkup(city):''}<p class="intel-note">${city.name===format('{home}')?format('{side}先遣旅的出发地与补给源。'):city.name===format('{target}')?format('沿据点间道路巡行，可返回{home}城整备。'):'国家会自主经营与守城，战后归属和守军随战果变化。'}</p><div class="intel-actions">${button('city-officers','驻将名录 · '+officersInCity(city.name,getSimulation()).length)}${button('locate-city','地图定位')}${button('plan-city',steps===0?'已在此地':'移动至此',!route||steps===0||w.unit.morale<=0)}</div></section></div>`;
  }else if(view==='officer'){
   html=`${format("<div class=\"personal-banner\"><div class=\"dossier-portrait\"><img src=\"{portrait}\" alt=\"主角演练形象\"></div><div><small>你扮演的{officer}</small><h3>{hero}</h3><p>{ruler}麾下 · 先遣旅指挥</p></div><span class=\"mode-tag\">单{officer}模式</span></div><dl class=\"intel-stats four\">")}${datum('可指挥兵力',format('{troops}'))}${datum('所在位置',c.terrain+' · '+c.q+','+c.r)}${datum('部队士气',w.unit.morale)}${datum('补给状态',supply?'连通':'中断')}</dl><h4>当前可下达的指令</h4><div class="personal-orders"><button data-action="choose-march"><b>行 军</b><span>右键地图目的地，立即下令并开始移动。</span></button><button data-action="standby"><b>原地待命</b><span>停止推进，撤销本队尚未完成的路线。</span></button><button data-action="return" ${w.unit.cell===w.origin.id||w.unit.morale<=0?'disabled':''}${format("><b>返回{home}</b><span>立即沿可达路线返回{home}。</span></button></div><p class=\"intel-note\">当前身份可指挥本队；城市任免、全势力外交不属于本队指令。{hero}为原创{side}士人。</p><details class=\"future-roles\"><summary>后续角色玩法方向</summary><p>拟接入：交游拜访、修习能力、接受任务、仕官与升迁；获得职位后逐步开放内政与提案。这些玩法尚未接入三维样区。</p></details>")}`;
  }else if(view==='roster'){
   html=rosterMarkup(selectedOfficer,rosterOptions,getSimulation());
  }else if(view==='unit'){
   const armies=getSimulation().actors.filter(a=>a.kind==='army'&&a.active!==false);
   html=`<div class="roster-toolbar"><div><strong>在编部队</strong><span>${armies.length+1} 队</span></div><span>点击部队，定位查看军情</span></div><div class="officer-table-scroll"><table class="officer-table army-ledger"><thead><tr><th>主将</th><th>势力</th><th>兵力</th><th>军令</th><th>目标 / 路程</th><th>查看</th></tr></thead><tbody><tr><th>${button('locate-unit',format('{hero} · 本队'))}${format("</th><td>{faction}</td><td>{troops}</td><td>")}${w.unit.path.length?'行军':'待命'}</td><td>${w.unit.path.length?'剩余 '+w.unit.path.length+' 格':'原地驻守'}</td><td>${button('locate-unit','定位')}</td></tr>${armies.map(a=>`<tr><th>${button('watch-'+a.id,a.name)}</th><td><i class="realm-dot" style="background:${faction(a.owner).color}"></i>${owner(a.owner)}</td><td>${a.troops.toLocaleString()}</td><td>${a.phase}</td><td>${a.destination} · ${a.path.length} 格</td><td>${button('watch-'+a.id,'观察')}</td></tr>`).join('')}${format("</tbody></table></div><p class=\"intel-note\">独自游历的{officer}请查阅「{officer}」。国家部队可迎战、攻城与撤退；主角本队暂不参加自动战斗。</p>")}`;
  }else{
   html='<p class="intel-lead">行军纪事 <span>最近 100 条 · 重置演练后清空</span></p><ol class="journal-list">'+entries.slice().reverse().map(e=>`<li><span>第 ${e.turn} 旬</span><p>${escape(e.text)}</p></li>`).join('')+'</ol>';
  }
 ...
 dialog.addEventListener('click',e=>{const cities=getSimulation().campaign?.cities||initialCities;
  ...
  else if(action==='return'){close();plan(cities.find(c=>c.name===scenario.player.home));}
 });
 return{open,openOfficer(id){rosterOptions.city='all';rosterOptions.faction='all';selectedOfficer=id;open('roster');content.querySelector('tr.is-selected')?.scrollIntoView({block:'nearest'});},refresh:render,record(text){entries.push({step:getWorld().unit.steps,turn:getSimulation().campaign?.turn||0,text});entries=entries.slice(-100);render();},reset(){entries=[];render();}};
}
