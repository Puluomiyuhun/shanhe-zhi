...
export function createIntelligence({getWorld,getSimulation,inspectActor,cities,pause,focusCity,focusUnit,plan,standby}){
 ...
 const tabs=[['realm','势力'],['cities','城市'],['officer','人物'],['roster',format('{officer}')],['unit','部队'],['journal','军情']];
 ...
 function render(){
  ...
  document.getElementById('intelTitle').textContent={realm:'势力情报',cities:'城市情报',officer:'人物与指令',roster:format('{officer}名录'),unit:'部队情报',journal:'军情簿'}[view];
  for(const b of dialog.querySelectorAll('[data-view]'))b.setAttribute('aria-pressed',String(b.dataset.view===view));
  let html='';
  if(view==='realm'){
   html=format('<div class="roster-toolbar"><div><strong>天下势力</strong><span>{factionCount} 方势力 · 演练配置</span></div><span>选择势力，查阅所属城市</span></div><div class="officer-table-scroll"><table class="officer-table army-ledger"><thead><tr><th>势力</th><th>主政人物</th><th>区域</th><th>控制土地</th><th>城池</th><th>情报</th></tr></thead><tbody>');
