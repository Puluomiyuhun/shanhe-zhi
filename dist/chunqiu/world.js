import {waterAt,crossingAt,passAt,passBlend} from './geography.js';
...
export function height(x,z){
 ...
 const mountain=qinling+taihang+yanshan+sichuan+daba+wushan+longshan+fangcheng;
...
export function createWorld(){const cells=[],lookup=new Map();for(let row=0;row<ROWS;row++)for(let col=0;col<COLS;col++){const r=row-Math.floor(ROWS/2),q=col-Math.floor(COLS/2)-Math.floor(r/2),x=DX*(q+r/2),z=DZ*r,y=height(x,z),water=waterAt(x,z),river=water.shore<.6,bridge=river&&!!crossingAt(x,z);const pass=passAt(x,z),road=!!pass;const terrain=bridge?'津渡':river?'河流':pass?'关隘':y>5.3?'险山':y>2.5?'山地':road?'官道':Math.sin(x*.4+z*.2)>.65&&Math.abs(x)>8?'林地':'平原';const c={id:cells.length,q,r,x,z,y,terrain,feature:bridge?crossingAt(x,z).name:river?water.name:pass?.name||'',walkable:!['河流','险山'].includes(terrain),cost:terrain==='官道'?.65:terrain==='山地'?2.4:terrain==='林地'?1.8:1,owner:initialOwner(x,z)};cells.push(c);lookup.set(q+','+r,c);}
