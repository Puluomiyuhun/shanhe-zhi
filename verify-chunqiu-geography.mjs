...
import {rivers,waterAt,crossings,passes} from './dist/chunqiu/geography.js';
const w=createWorld();
...
console.log(JSON.stringify({riverSystems:4,ferries:crossings.length,passes:passes.length,riverCells:w.cells.filter(c=>c.terrain==='河流').length,qinShuSteps:route.path.length-1,closedPassDetour:detour?.path.length-1||null,closedFerriesBlockQinJin:true}));
